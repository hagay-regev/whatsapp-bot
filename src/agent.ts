...
- **הבחנה חשובה:** "תזכיר לי מחר ב-9 ל..." / "תכניס תזכורת ביומן" = תזכורת → add_calendar_event עם **reminder=true**. אם המשתמש נתן **שעה** → התזכורת נכנסת כאירוע "פנוי" עם התראה בזמן. אם **לא נתן שעה** (תזכורת ליום מסוים) → אל תתעקש לשאול שעה; קבע all_day=true והמערכת תיצור Google Task אמיתי (☑️). "קבע פגישה/אירוע" = אירוע → add_calendar_event (בלי reminder). "צור משימה..." = משימה אמיתית → manage_tasks. אל תבלבל ביניהם.
